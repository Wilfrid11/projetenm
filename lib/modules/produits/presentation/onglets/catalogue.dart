...
class CatalogueScreen extends StatelessWidget {
  ...
  const CatalogueScreen({super.key, required this.controller, required this.user});

  @override
  Widget build(BuildContext context) {
    bool estAdmin = user.role == "admin";
    ...
    return Scaffold(
      ...
      body: listeProduits.isEmpty
          ? const Center(child: Text("Aucun article dans le catalogue."))
          : ListView.builder(
                ...
                return Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text("Réf: ${prod.reference} | Catégorie: ${prod.categorie}", style: const TextStyle(color: Colors.grey, fontSize: 12)),
                      const Divider(height: 20),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Text("Prix Vente: ${prod.prixVente.toInt()} F", style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF1A3B8B))),
                          Text("Seuil Alerte: ${prod.seuilAlerte} u", style: const TextStyle(color: Colors.orange)),
                        ],
                      )
                    ],
                  ),
  ...
  void _ouvrirFormulaireCreation(BuildContext contexte) {
    ...
    showModalBottomSheet(
      ...
      builder: (contexte) {
        return Padding(
          padding: EdgeInsets.only(bottom: MediaQuery.of(contexte).viewInsets.bottom, top: 20, left: 16, right: 16),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    if (nomCtrl.text.isNotEmpty && refCtrl.text.isNotEmpty) {
                      controller.creerFicheCatalogue(
                        Produit(
                          nom: nomCtrl.text,
                  ...
                  child: const Text("Enregistrer la fiche", style: TextStyle(color: Colors.white)),
                ),
              ),
              const SizedBox(height: 20),
            ],
